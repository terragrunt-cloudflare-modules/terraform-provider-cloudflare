...
// TestAccZeroTrustAccessAIControlsMcpPortal_serversSetOrderIndependent covers
// the fixes in this resource's servers modeling:
//   - server_id is read back from the API (the API returns identity under
//     "id"; the schema/write path use "server_id"). Without the read-side
//     mapping, server_id is null and every plan shows "+ server_id".
//   - servers is a set keyed by server_id, so reordering servers[] in
//     configuration produces no diff (the backend ignores client write order
//     and returns a canonical order, so a list would churn).
//   - description omitted (null) matches the API-persisted "" (no diff).
func TestAccZeroTrustAccessAIControlsMcpPortal_serversSetOrderIndependent(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// description must round-trip as "" though omitted.
			{
			...
			// Re-apply identical config: no diff (server_id stable, no
			// "+ server_id", no description churn).
			{
				Config: acctest.LoadTestCase("servers_set.tf", accountID, domain, name),
